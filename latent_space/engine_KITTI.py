...
from dataset import KITTI_dataloader
from model import KITTI_latent_model
...
class Engine:
    def __init__(self, args, logger):
        ...
        self.dataset = KITTI_dataloader(self.args, self.mode)
        self.model = KITTI_latent_model(
            self.num_ensemble,
            self.win_size,
            self.dim_x,
            self.dim_z,
    ...
    def test(self):
        ...
        test_dataset = KITTI_dataloader(self.args, "test")
        ...
        save_path = os.path.join(
            ...
            "v-KITTI-result-{}.pkl".format(self.global_step),
        )
            ...
            if self.global_step != 0 and (epoch + 1) % self.args.train.save_freq == 0:
                ...
                torch.save(
                    ...
                    os.path.join(
                        ...
                        self.args.train.model_name,
                        "v-KITTI-model-{}".format(self.global_step),
                    ),
                )
                torch.save(
                    checkpoint_tmp,
                    os.path.join(
                        self.args.train.log_directory,
                        self.args.train.model_name,
                        "v-KITTI-support-model-{}".format(self.global_step),
                    ),
                )
