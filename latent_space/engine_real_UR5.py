...
from dataset import UR5_real_dataloader
from model import UR5_latent_model
from model import UR5real_latent_model
...
class Engine:
    def __init__(self, args, logger):
        ...
        self.model = UR5real_latent_model(
            self.num_ensemble,
            self.win_size,
            self.dim_x,
            self.dim_z,
    ...
    def test(self):
        ...
        save_path = os.path.join(
            ...
            "v-UR5-real-result-{}.pkl".format(self.global_step),
        )
    ...
    def modality_selection(self, data):
        _, gt_image, obs_1, _ = data
        full_modality = list(range(0, self.sensor_len))
        obs = [gt_image, obs_1]

        p = random.uniform(0, 1)
        if p < 0.5:
            mod_list = sorted(random.sample(full_modality, 1))
            list_inputs = [obs[mod_list[0]]]
        else:
    ...
    def loss_calculation(self, output, gt):
        ...
        if len(output[-2]) >= 3:
            ...
            add1_loss = add1_loss + self.criterion(output[-2][0], gt)
            ...
            if self.global_step != 0 and (epoch + 1) % self.args.train.save_freq == 0:
                ...
                torch.save(
                    ...
                    os.path.join(
                        ...
                        self.args.train.model_name,
                        "v-UR5-real-model-{}".format(self.global_step),
                    ),
                )
                torch.save(
                    checkpoint_tmp,
                    os.path.join(
                        self.args.train.log_directory,
                        self.args.train.model_name,
                        "v-UR5-real-support-model-{}".format(self.global_step),
                    ),
                )
